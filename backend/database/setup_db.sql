...
-- Final Modular Structure (5 Core Tables)
...
CREATE TABLE IF NOT EXISTS health_reports (
    ...
    patient_identity_json LONGTEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (patient_id) REFERENCES patients(id) ON DELETE CASCADE
);

-- 4. Medical Metrics Data Table (Stores individual lab results)
CREATE TABLE IF NOT EXISTS medical_metrics_data (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS followup_comparisons (
    ...
    activity_improvement FLOAT DEFAULT 0.0,
    wellness_improvement FLOAT DEFAULT 0.0,
    decline_percentage FLOAT DEFAULT 0.0,
    health_trend VARCHAR(50), -- Improving, Stable, Deteriorating
    comparison_summary TEXT,
    comparisons_json LONGTEXT,
    FOREIGN KEY (patient_id) REFERENCES patients(id) ON DELETE CASCADE,
